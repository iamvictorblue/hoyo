...
final class GameScene: NSObject, SCNSceneRendererDelegate {
    ...
    /// script.
    // propHash moved to PropMeshes, alongside the geometry that uses it.
    ...
    private func boulderGeometry(variant: Int, material: SCNMaterial) -> SCNGeometry {
        let m = PropMeshes.boulder(variant: variant)
    ...
    private func palmGeometries(variant: Int, trunkMaterial: SCNMaterial,
        ...
        let m = PropMeshes.palm(variant: variant)
    ...
    private func vegetation(_ parent: SCNNode) {
        ...
        for (container, label) in [(palmContainer, "palm trunks"),
                                   (frondContainer, "palm fronds")] {
            let flat = FlattenGuard.flattened(container, label)
        ...
        for k in 0..<rockTarget {
            ...
            rock.eulerAngles.x = (PropMeshes.propHash(k, 61) - 0.5) * 0.5
            rock.eulerAngles.z = (PropMeshes.propHash(k, 67) - 0.5) * 0.5
            rockContainer.addChildNode(rock)
        }
        parent.addChildNode(FlattenGuard.flattened(rockContainer, "boulders"))
